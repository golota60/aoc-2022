...
type TreeState = Record<string, string>;
...
let parsedFolders: Array<Folder> = [];
...
input.split("\n").reduce((acc, cmdLine) => {
  if (cmdLine.startsWith("$")) {
    // input
    const [_, command, arg] = cmdLine.split(" ");
    if (command === "ls") {
      // do nothing
      return acc;
    }
    if (command === "cd") {
      ...
      if (arg === "..") {
        ...
        currMarker = currMarker + arg + (arg !== "/" ? "/" : "");

        // init new dir
        parsedFolders.push({ path: currMarker, size: 0 });
        return acc;
      }
    }
    throw new Error("$ IS FUCKED");
  } else {
    // output
    const [firstArg, secondArg] = cmdLine.split(" ");
    if (firstArg === "dir") {
      // do nothing
    } else {
